...
class ResNetD(nn.Module):
    def __init__(self, model_name: str = '50') -> None:
        ...
        # self.attention_1=AxialAttention(dim=64,heads=8,dim_index=1)
        # self.attention_2 = AxialAttention(dim=128, heads=8, dim_index=1)
        # self.attention_3 = AxialAttention(dim= 256, heads=8, dim_index=1)
        # self.attention_4 = AxialAttention(dim=512, heads=8, dim_index=1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # x=# [1, 64/256, H/4, W/4]
        x2 = self.layer2(x1)
        # x2=self.attention_2(x2)# [1, 128/512, H/8, W/8]
        x3 = self.layer3(x2)
        # x3=self.attention_3(x3)# [1, 256/1024, H/16, W/16]
        x4 = self.layer4(x3)
        # x4=self.attention_4(x4)# [1, 512/2048, H/32, W/32]
        return x1, x2, x3, x4
