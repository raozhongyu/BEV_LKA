...
class Cross_Attention_Douple(nn.Module):
    def __init__(self,
                 bev_input_channel: int,
                 rv_input_channel: int,
                 embed_dim: int,
                 num_heads: int,
                 bev_size: Sequence[int] = (32, 32),
                 bev_block_res: Sequence[int] = (2, 2),
                 rv_size: Sequence[int] = (32, 32),
                 rv_block_res: Sequence[int] = (2, 2),
                 hidden_channels: Sequence[int] = 128):
        r"""
...
if __name__ == '__main__':
    ...
    se =Cross_Attention_Decouple(bev_input_channel=128,rv_input_channel=128,embed_dim=128,num_heads=2)
    outputs,atten_maps = se([input_1,input_2])
    print(outputs.shape)